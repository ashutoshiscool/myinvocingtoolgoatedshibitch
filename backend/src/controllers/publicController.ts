...
import { createPaypalOrder, capturePaypalOrder, getPaypalConfig } from '../services/paypalService';
...
export async function getPublicInvoice(req: Request, res: Response) {
  ...
  try {
    ...
    const showRealPaypal = paypalConfig.mode !== 'simulation' && !!(paypalConfig.clientId && paypalConfig.clientSecret);
    ...
    res.json({
      ...
      company,
      transactions,
      paypalClientId: showRealPaypal ? paypalConfig.clientId : 'test',
      hasPaypalKeys: showRealPaypal
    });
  } catch (error) {
...
export async function downloadPublicPDF(req: Request, res: Response) {
  const { secure_hash } = req.params;
...
export async function createOrder(req: Request, res: Response) {
  ...
  try {
    ...
    // Check if PayPal config is present and not in simulation mode
    const paypalConfig = await getPaypalConfig();
    const showRealPaypal = paypalConfig.mode !== 'simulation' && !!(paypalConfig.clientId && paypalConfig.clientSecret);
    
    if (!showRealPaypal) {
      // Return a simulated order ID to let them test payment without setup
      console.warn('PayPal running in Simulation Mode. Creating a mock order for testing.');
      return res.json({ id: `MOCK_ORDER_${Date.now()}`, isMock: true });
    }

    const order = await createPaypalOrder(invoice.grand_total, invoice.currency, invoice.invoice_number);
